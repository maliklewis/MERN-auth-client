...
const Admin = ({history}) => {
    ...
    const loadProfile = () => {
        ...
        .then(response => {
            console.log('ADMIN PROFILE RETRIEVAL SUCCESS', response);
            const {role, name, email} = response.data;
            setValues({...values, role, name, email})
        })
        .catch(error => {
            console.log('ADMIN PROFILE RETRIEVAL ERROR', error.response.data.error);
    ...
    const clickSubmit = event => {
        ...
        axios({
            method: 'PUT',
            url: `${getLocalStorage('server-url')}/admin/update`,
            data: {name, password},
            headers: {
                Authorization: `Bearer ${token}`
            }
